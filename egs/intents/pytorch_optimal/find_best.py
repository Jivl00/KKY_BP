...
with open('temp/experiments.json', 'r', encoding='utf-8') as f:
    ...
    print(len(file))
    best_models = []
    best_acc = max([file[key]['accuracy'] for key in file])
    for key in file:
        if file[key]['accuracy'] == best_acc:
...
for model in best_models:
    model_name = model
    NET_PARAMS = {
        'hidden_layers': [file[model]['hidden_layers']],
        'learning_rate': file[model]['learning_rate'],
        'epochs': file[model]['epochs'],
        'batch_size': file[model]['batch_size'],
        'do_fit': True,
        'overwrite_best_model': False
    }
    opt = file[model]['optimizer']
    accuracies = []
    times = []
    for i in seed:
        ...
        if count % 100 == 0:
            print(count)
    with open('temp/bests.json', 'w', encoding='utf-8') as f:
        output[model_name] = {'accuracy': accuracies, 'times': times}
