...
def search(google_question):
    ...
    results = [soup.find('tr', class_='ztXv9').find('th')
               ...
               soup.find('ol', class_='X5LH0c'),
               soup.find('div', class_='dbg0pd'),
               soup.find('ul', class_='i8Z77e'),
               soup.find('div', class_='dDoNo vrBOv vk_bk'),
               soup.find('div', class_='gsrt vk_bk FzvWSb YwPhnf'),
               soup.find('div', class_='LGOjhe'), soup.find('span', class_='ILfuVd'), soup.find('a', class_='FLP8od'),
               soup.find('div', class_='kp-hc'),
               soup.find('div', class_='zCubwf'),
               soup.find('div', class_='ayqGOc kno-fb-ctx kpd-lv kpd-le KBXm4e'),
               soup.find('div', class_='wob_dcp'), soup.find('div', class_='dDoNo ikb4Bb gsrt'),
               soup.find('div', class_='Z0LcW t2b5Cf'), soup.find('div', class_='ayqGOc kno-fb-ctx KBXm4e'),
               soup.find('div', class_='sXLaOe')]
    out = ''
    for result in results:
        if result is not None:
            out = result.text
    if out == '':
        out = 'Neznám odpověď. Polož jednodušší otázku, prosím.'
    print(out)
    print()
    return out
